...
<style>
    ...
    .main-header { font-size: 32px; font-weight: 900; color: #cc0000; text-align: center; text-transform: uppercase; margin-bottom: 20px; text-shadow: 1px 1px 1px #ddd; }
    .marquee-container { width: 100%; overflow: hidden; background-color: #fff5f5; border: 1px solid #cc0000; padding: 10px 0; margin-bottom: 20px; border-radius: 5px; }
    .marquee-text { font-size: 18px; font-weight: bold; color: #cc0000; white-space: nowrap; animation: marquee 25s linear infinite; }
    @keyframes marquee { 0% { transform: translateX(100%); } 100% { transform: translateX(-100%); } }
    .section-header { font-size: 20px; font-weight: bold; color: #006633; border-bottom: 2px solid #006633; margin-top: 20px; margin-bottom: 10px; padding-bottom: 5px; }
    .guide-box { background-color: #f4fcf6; border: 1px solid #006633; border-radius: 5px; padding: 20px; font-size: 16px; line-height: 1.6; }
    .footer { position: fixed; left: 0; bottom: 0; width: 100%; background-color: #006633; color: white; text-align: center; padding: 10px; font-size: 14px; z-index: 9999; }
    .stButton>button { background-color: #006633; color: white; font-size: 18px; border-radius: 5px; height: 50px; border: none; }
...
def get_selected_context(folder_path, selected_files):
    ...
    for file_name in files_to_read:
        ...
        if os.path.exists(full_path):
            all_text += f"\n--- TÀI LIỆU CĂN CỨ: {file_name} ---\n{read_doc_text(full_path)[:20000]}\n"
    return all_text

# --- 4. HÀM AI (LÕI MỚI - TỰ ĐỘNG DÒ TÌM MODEL CHỐNG LỖI 404) ---
def generate_test_v5(mon, lop, loai, context):
    ...
    # AI sẽ tự động thử các danh sách tên model từ mới nhất đến cũ nhất
    ...
    for m in models_to_try:
        try:
            ...
            last_error = str(e)
            continue # Nếu bị lỗi 404 tên này, tự động thử tên tiếp theo
...
st.markdown('<div class="marquee-container"><div class="marquee-text">🌸 CUNG CHÚC TÂN XUÂN CHÀO NĂM BÍNH NGỌ 2026 - CHÚC QUÝ THẦY CÔ VÀ CÁC EM HỌC SINH NĂM MỚI THÀNH CÔNG RỰC RỠ 🌸</div></div>', unsafe_allow_html=True)

with st.expander("📖 BẤM VÀO ĐÂY ĐỂ XEM HƯỚNG DẪN SỬ DỤNG CHI TIẾT", expanded=False):
    st.markdown("<div class='guide-box'><b>BƯỚC 1: THIẾT LẬP THÔNG TIN (Cột trái)</b><br>Chọn Cấp học, Lớp, Môn học để mở kho dữ liệu tương ứng.<br><br><b>BƯỚC 2: TẢI TÀI LIỆU (Cột trái)</b><br>Tải Ma trận, Đề minh họa hoặc Nội dung ôn tập lên kho.<br><br><b>BƯỚC 3: CHỌN TÀI LIỆU & TẠO ĐỀ (Cột phải)</b><br>Tích chọn các file muốn sử dụng, chọn loại đề và bấm nút Tạo đề.</div>", unsafe_allow_html=True)
...
with col2:
    try: files_in_dir = [f for f in os.listdir(curr_dir) if f.endswith(('.docx', '.pdf', '.txt'))]
    except: files_in_dir = []
    ...
    if st.button("🚀 BẮT ĐẦU TẠO ĐỀ NGAY"):
        if not selected_files: st.error("Vui lòng chọn tài liệu trước!")
        else:
            context = get_selected_context(curr_dir, selected_files)
            with st.spinner("Đang dò tìm phiên bản AI phù hợp và soạn đề (Vui lòng đợi 10-20 giây)..."):
...
st.markdown('<div class="footer">Ứng dụng tạo đề kiểm tra được tạo bởi thầy Phan Quốc Khánh và trợ lý ảo Gemini - Trường Tiểu học Hua Nguống.<br>Số điện thoại: 0389655141</div>', unsafe_allow_html=True)